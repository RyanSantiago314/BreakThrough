...
public class MovementHandler : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if(other.CompareTag("Player") && other.gameObject.transform.parent.name == opponent.gameObject.transform.parent.name)
        {
            ...
            if (((!Actions.airborne && opponentMove.Actions.airborne  && !hittingWall)|| (Actions.airborne && opponentMove.hittingWall))  && opponentMove.rb.velocity.y <= 0)
            {
    ...
    void WallStates()
    {
        ...
        else if (Actions.wallBounce && HitDetect.hitStun > 0  && transform.position.y > 1.4f)
        {
            Actions.groundBounce = false;
            Actions.wallBounce = false;
            rb.velocity = Vector2.zero;
