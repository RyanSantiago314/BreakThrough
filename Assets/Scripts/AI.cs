...
public class AI : MonoBehaviour
{
    ...
    void moveCloser()
    {
        ...
        //Foward Dash
        if (distanceBetween >= 2 && rand.Next(3) == 1)
        {
            if(faceLeft == true)
            {
                MaxInput.moveLeft("Player2");
        ...
        if (p2y < p1y - 0.5 && rand.Next(0,4) == 1)
        {
            MaxInput.Jump("Player2");
            isJumping = true;
            if(rand.Next(0,3) == 1)
    ...
    void updateProperties()
    {
        ...
        distanceBetween = Math.Abs(p1x - p2x);
    }
